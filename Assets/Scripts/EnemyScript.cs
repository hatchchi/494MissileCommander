...
public class EnemyScript : MonoBehaviour
{
    ...
    public List<int[]>
    PlaceEnemyShips() ///asigns all five ships (noses) to a tile number, and stores under List<int>
    {
        List<int[]> enemyShips =
            new List<int[]> {
            ...
            while (taken == true)
            {
                ...
                for (int i = 0; i < tileNumArray.Length; i++)
                {
                    ...
                    )
                    {
                        ...
                        minusAmount == 1 &&
                        shipNose / 10 != ((shipNose - i * minusAmount) - 1) / 10
                    )
                    {
                        taken = true;
                        break;
                ...
                if (taken == false)
                {
                    ...
                    {
                        tileNumArray[j] =
                            gridNumbers[shipNose - j * minusAmount];
    ...
    public void NPCTurn()
    {
        ...
        if (
            hitIndex.Count > 1 //if there have been MORE THAN ONE prior hit in this round, (we would already know whether to choose x or y direction)
        )
        {
            ...
            {
                if (
                    guessGrid[nextIndex] == 'm' ||
                    nextIndex > 100 ||
                    nextIndex < 0 //if not already marked missed, OR not off the board
                )
                {
            ...
            hitIndex.Count == 1 //if there was already ONE HIT in this round (we won't know whether to go up or down)
        )
        {
            ...
            closeTiles.Add(-1);
            closeTiles.Add(10);
            closeTiles.Add(-10); //adjust each to be one over, up or down
            ...
            while ((!onGrid || guessGrid[possibleGuess] != 'o') &&
                closeTiles.Count > 0
            )
            {
                //if its off the grid OR the tile marked 'already sunk(?), AND random choice was positive...
        ...
        } // if this is a blind attempt to find a ship in this round
        else
        {
            int nextIndex = Random.Range(0, 100); //any number 0-100

            while (guessGrid[nextIndex] != 'o')
            nextIndex = Random.Range(0, 100); //continue until choice is not already fully sunken ship
            nextIndex = GuessAgainCheck(nextIndex); //check that its not an edge case, below
            Debug.Log(" --- need to guess twice ");
            nextIndex = GuessAgainCheck(nextIndex); //check that its not an edge case, below
            Debug
                .Log(" -Checking/adjusting a second time around (will catch an issue if it was in the corner");

            guess = nextIndex; //new guess is now calculated (87, etc)  'guess' is used in resolving section below the if else statements.
        }

        GameObject tile = GameObject.Find("Tile (" + (guess + 1) + ")"); //why guess +1? - maybe because guesses run from zero and not from one like tiles?
        ...
        Debug
            .Log("this is the tile transform position:" +
        ...
        GameObject missile =
            ...
            vec,
            enemyMissilePrefab.transform.rotation);

        missile.GetComponent<EnemyMissileScript>().SetTarget(guess); //in other script it inputs 'guess' value to the variable 'targetTile'
        missile.GetComponent<EnemyMissileScript>().targetTileLocation =
            tile.transform.position; //inputs tile position to variable 'targetTileLocation'
    }

    private int
    GuessAgainCheck(int nextIndex) //checks if on the edges, and tries again
    {
        ...
        bool edgeCase =
            ...
            nextIndex > 89 ||
            nextIndex % 10 == 0 ||
            nextIndex % 10 == 9; //edge case means 0-10 or 90-100 or a x-10-number, or a x-1-number
        ...
        Debug
            ...
            edgeCase +
            " g:" +
            nearGuess); //reports:  nx: 15 newGuess: 15 e:False g:False
        return newGuess;
    }
